...
if [ -z "$TARGET_ARCH" ]; then
 ...
 printf 'With a x86/64 machine, selecting other archs like arm, mips etc will return error\n\n'
 printf 'x86 BASED\n'
...
apt -y install pax-utils ncurses-dev bison texinfo flex autoconf automake libtool patch curl cvs build-essential subversion gawk gperf libncurses5-dev libexpat1-dev vim
...
if [ ! -d $STORAGE_APORTS ]; then
    echo "Aports does not exist proceed with cloning"
else
