...
cross-compile an Linux base system for new architecture.

You will need to pass two parameters
- Architecture, for example arch64 as first argument
- Directory of aports, for example /root/aports as second argument
- Use git clone git://git.alpinelinux.org/aports.git
...
by running this with the target arch as parameter, e.g.:
	./$program aarch64 /root/aports
EOF
	return 1
fi
echo Architecture selected is $TARGET_ARCH
...
echo CBUILDROOT is $CBUILDROOT and CBUILD_ARCH is $CBUILD_ARCH
...
if [ ! -d "$CBUILDROOT" ]; then
		...
		apk add --quiet --initdb --arch $TARGET_ARCH --root $CBUILDROOT
fi
...
[ -e "$APORTS/main/build-base" ] || die "Unable to deduce aports base checkout, please set aports directory as second argument"

echo Aports directory is set to $APORTS
